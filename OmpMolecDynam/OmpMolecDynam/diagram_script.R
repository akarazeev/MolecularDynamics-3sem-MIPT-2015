...
step <- 0
plot(temp$V2[step:nrow(temp)], temp$V1[step:nrow(temp)], type="l", main=mean(temp$V1[step:nrow(temp)]), xlab=100*max(abs(temp$V1[step:nrow(temp)]-mean(temp$V1[step:nrow(temp)])))/mean(temp$V1[step:nrow(temp)]))
...
par(new=TRUE)
plot(kinetic$V2, kinetic$V1, type="l", xlab="", ylab="", axes=FALSE, ylim=range(c(energy$V1,kinetic$V1,poten$V1)), col="red")
par(new=TRUE)
plot(poten$V2, poten$V1, type="l", xlab="", ylab="", axes=FALSE, ylim=range(c(energy$V1,kinetic$V1,poten$V1)), col="blue")
dev.off()
